	...
	Filter: {
		...
		filterByPriceType: function () {
			$("#store_realms .store_item").each(function () {
				...
				// Filter based on vp / dp toggling - now using buttons instead of anchoring links
				$(this).find(".store_buttons button").each(function () {
					if ($(this).hasClass("dp_button")) {
						hasDp = true;
					}
					else if ($(this).hasClass("vp_button")) {
						hasVp = true;
					}
				});
		...
		updatePrices: function () {
			...
			$("#store_realms .store_item").each(function () {
				$(this).find(".store_buttons button").each(function () // changed from a to button
				{
					if ($(this).hasClass("dp_button")) {
						price = parseInt($(this).find(".dp_price_value").html().replace(/[^0-9]+/, ""));
						if (price > Store.Customer.dp) {
							$(this).fadeTo(300, 0.5);
							$(this).attr('disabled', 'disabled'); // disable if they can't afford
						}
						else {
							$(this).fadeTo(300, 1);
						...
						price = parseInt($(this).find(".vp_price_value").html().replace(/[^0-9]+/, ""));
